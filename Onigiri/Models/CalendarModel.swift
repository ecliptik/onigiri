...
final class CalendarModel {
    ...
    func refresh(goal: SyncedGoal?, forceWeights: Bool = false) async {
        ...
        let plan = await DailyPlanLoader.load(
            goal: goal, burnCorrectionKcal: SharedStore.burnCorrectionKcal)
    ...
    /// The stored burn correction changed: every total already held was
    /// graded under the old one, including months loaded on demand, so
    /// drop them all rather than merge a refresh over them — a merged
    /// window over stale older months would grade one calendar two ways.
