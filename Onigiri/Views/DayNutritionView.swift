...
struct DayNutritionView: View {
    ...
    private var summarySection: some View {
        Section(dayLabel) {
            ...
            // burn a few kcal higher, with nothing on screen accounting for
            // the difference (the user, 2026-08-24).
            //
            // The measurement prints only past `creditNoteThresholdKcal`
            // — a gap of a few kcal is not a disagreement anyone would notice,
            ...
            // The burn correction — its OWN row, never folded into the
            // two above (`plans/PLAN-burn-correction.md`): into resting
            // it would take the row under its own floor, into active it
            // goes negative on a quiet day, and invisible it is exactly
            // the "silently" every rule here is written against. It
            // prints what actually LANDED today, so Active + Resting +
            // this is the burn Net is cut from; when the resting floor
            // held part of it back (a morning, resting credited from the
            // estimate), the accepted figure sits underneath, in the
            // rows' own credit-over-measurement grammar.
            if model.burnCorrectionKcal.rounded() != 0 || SharedStore.burnCorrectionKcal != 0 {
                iconRow("Burn correction", icon: { Image(systemName: "scalemass.fill").foregroundStyle(.secondary) }) {
                    let applied = model.burnCorrectionKcal
                    let accepted = SharedStore.burnCorrectionKcal
                    VStack(alignment: .trailing, spacing: 2) {
                        // A true minus, as Goal prints it.
                        Text("\(applied.formatted(.number.precision(.fractionLength(0))).replacingOccurrences(of: "-", with: "−")) kcal")
                            .monospacedDigit()
                        if abs(accepted - applied) >= DayBudget.creditNoteThresholdKcal {
                            Text("\(accepted.formatted(.number.precision(.fractionLength(0))).replacingOccurrences(of: "-", with: "−")) set")
                                .font(.caption)
                                .foregroundStyle(.secondary)
                                .monospacedDigit()
                        }
                ...
                .accessibilityIdentifier("burnCorrectionRow")
            }
            ...
            // "banked / goal" says on-track at a glance, where two separate
            ...
            // "-N / goal" reads like arithmetic nobody asked for.
