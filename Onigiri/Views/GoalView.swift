...
struct GoalView: View {
    ...
    /// The suggestion a tap on the offer (or on Recalculate) is asking
    /// about, held while the alert that states its consequence is up.
    /// The stored correction changes only from that alert's button.
    @State private var proposedCorrection: BurnCorrection.Suggestion?
    /// The accepted burn correction (`plans/PLAN-burn-correction.md`),
    /// observed so the rows below and the preview budget follow it.
    /// Written ONLY through `setBurnCorrection`.
    ...
    /// that costs. A morning reading just under the target rendered
    /// an orange "Target must be below your current weight." (raw)
    /// beside a full bar reading more lost than there was to lose (raw)
    /// beside no celebration at all (basis, still above the target) —
    /// three answers, one
    ...
    /// "Deficit needed" that the screen never showed, so the deficit
    ...
    /// into. Lowering the target 10 lb in the field above, with the
    /// stored date two weeks out, asked for a deficit hundreds of
    /// kcal/day larger than the day's whole burn: a NEGATIVE budget, so Today read
    /// "+N kcal over" at 9am with nothing eaten (the user,
    ...
    private var plan: CalorieBudget.Plan? {
        ...
        CalorieBudget.derivePlan(
            ...
            averageDailyBurnKcal: correctedAverageBurnKcal,
    ...
    /// Health's two-week average with the accepted burn correction
    /// applied, by the same rule every day's burn gets it
    /// (`BurnCorrection.apply`) — so the average-day preview and the
    /// days it previews are cut from one kind of burn. Through
    /// `derivePlan` this is also where the correction meets the two
    /// budget floors (`minReasonableBudget`, the resting estimate): a
    /// corrected average that leaves too little to eat raises the
    /// aggressive-pace warning like any other plan. The day path has no
    /// such warning by design (`completedDayPlan` — a spent day cannot
    /// act on it); there the correction's own floor is the resting
    /// estimate. nil stays nil: an ASSUMED 2,000 is not a measurement to
    /// correct.
    private var correctedAverageBurnKcal: Double? {
        model.averageBurnKcal.map {
            BurnCorrection.apply(
                toBurnKcal: $0,
    ...
    private func budgetSection(_ plan: CalorieBudget.Plan) -> some View {
        Section {
            ...
            // number describes. Without it, someone who reads one figure
            // here and a much lower one on Today has two answers to one question
    ...
    private var progressTotals: some View {
        if hasProgressTotals {
            ...
            if let predicted = model.trend.predicted30Lb, let actual = model.trend.actual30Lb {
                ...
                // The one figure that makes the two numbers above
                // legible, and the reason it is HERE: a cross-check is
                // unreadable away from the pair it checks. It first
                // shipped inside "How your budget is calculated"
                // (2026-08-18) and was cut from it six days later —
                // there it was a second measured burn in a column of
                // descending figures and read as a DERIVATION, under a
                // heading promising one calculation. Beside the two
                // numbers it reconciles it reads as what it is
                // (returned 2026-09-22, the placement that cut named).
                //
                // REPORTED, never planned from: `ObservedBurn` lists the
                // five causes this single number cannot tell apart, and
                // feeding it back into `dayBurn` would rebuild the
                // trailing-average substitution PLAN-earned-budget
                // deleted, silently.
                //
                // No delta row beside it. A companion "N below measured"
                // was built and removed the same day it was written: the
                // honest basis for that subtraction is the mean over
                // these same tracked days, which is not the burn shown
                // anywhere, so it contradicted the row it existed to
                // explain.
                //
                // nil is SILENCE, never a zero — under
                // `ObservedBurn.minimumTrackedDays` the mean intake of
                // the tracked days stops standing in for the days the
                // scale actually moved across.
                if let observed = model.trend.observedBurnKcal {
                    LabeledContent("Burn, from the scale") {
                        Text("≈ \(observed, format: .number.precision(.fractionLength(0))) kcal/day")
                            .monospacedDigit()
                    }
                    // "Energy" in the caption, "Burn" in the row label
                    // above — the register split (2026-08-02). Naming
                    // the three causes is the caption's real job: the
                    // number invites "my watch is lying", which is one
                    // of them and cannot be told from the other two.
                    Text("What your weigh-ins imply you burn; under-logged food, Health reading high and water weight all look the same here.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                    // The OFFER, beside the number it comes from — and
                    // only past the same 21-day gate, since it sits
                    // inside this `if let`.
                    if burnCorrectionKcal == 0 {
                        burnCorrectionOffer
                    }
                }
            }
        }
        // A correction in force shows whether or not the offer's gate
        // still holds: it is live on every day's burn, so it must stay
        // visible and removable in one tap.
        if burnCorrectionKcal != 0 {
            burnCorrectionStatus
        }
    }

    /// "Correct Burn by −N kcal/day…" — the app proposes, the person
    /// decides, the way the finish-line date button works.
    ///
    /// The caption is the requirement the sequencing decision put on
    /// this build (`plans/PLAN-burn-correction.md`, Hazards): the app
    /// cannot enforce the fortnight of weighed logging, so the offer has
    /// to say what it was calibrated against and over how many days,
    /// and that it absorbs under-logging too. A bare "Use −300?" invites
    /// exactly the early tap that decision exists to delay.
    @ViewBuilder
    private var burnCorrectionOffer: some View {
        if let suggestion = model.trend.burnCorrectionSuggestion {
            Button("Correct Burn by \(signedKcal(suggestion.kcalPerDay)) kcal/day…") {
                proposedCorrection = suggestion
            }
            .accessibilityIdentifier("burnCorrectionOffer")
            Text(correctionBasis(suggestion))
                .font(.caption)
                .foregroundStyle(.secondary)
        }
    }

    /// The correction in force: its value, how old it is, and one tap
    /// out. Re-suggesting is a BUTTON — the stored value never follows
    /// the scale on its own (that would be the adaptive TDEE
    /// PLAN-earned-budget deleted) — and it only appears when the scale
    /// now says something materially different.
    @ViewBuilder
    private var burnCorrectionStatus: some View {
        LabeledContent("Burn correction") {
            Text("\(signedKcal(burnCorrectionKcal)) kcal/day")
                .monospacedDigit()
        }
        .accessibilityIdentifier("burnCorrectionStatus")
        Text(correctionInForceCaption)
            .font(.caption)
            .foregroundStyle(.secondary)
        if let suggestion = model.trend.burnCorrectionSuggestion,
           abs(suggestion.kcalPerDay - burnCorrectionKcal) >= BurnCorrection.minimumOfferKcal {
            Button("Recalculate: \(signedKcal(suggestion.kcalPerDay)) kcal/day…") {
                proposedCorrection = suggestion
            }
            Text(correctionBasis(suggestion))
                .font(.caption)
    ...
    private var correctionInForceCaption: String {
        let set = SharedStore.burnCorrectionSetAt
            .map { "Set \($0.formatted(.dateTime.month(.abbreviated).day())); " } ?? ""
        let direction = burnCorrectionKcal < 0 ? "lowers" : "raises"
        return "\(set)\(set.isEmpty ? direction.capitalized : direction) every day's burn, past days included."
    }

    /// What the suggestion was worked out from, in the numbers it was
    /// worked out from — so its age and depth are read before it is
    /// taken, and its blind spot is named.
    private func correctionBasis(_ suggestion: BurnCorrection.Suggestion) -> String {
        let days = suggestion.trackedDays
        let since = suggestion.since.formatted(.dateTime.month(.abbreviated).day())
        let measured = suggestion.measuredBurnKcal.formatted(.number.precision(.fractionLength(0)))
        let scale = suggestion.scaleBurnKcal.formatted(.number.precision(.fractionLength(0)))
        return "From \(days) tracked days since \(since): Health measured ≈ \(measured) kcal/day, the scale implies ≈ \(scale)."
    }

    /// Signed with a true minus, never a bare number: which way the
    /// correction moves the burn is half of what it says.
    private func signedKcal(_ kcal: Double) -> String {
        let magnitude = abs(kcal).formatted(.number.precision(.fractionLength(0)))
        return kcal < 0 ? "−\(magnitude)" : "+\(magnitude)"
    }

    /// The ONE writer from this screen. Everything that composes a burn
    /// reads the stored value, so after writing it: the watch and the
    /// widgets (the push carries both keys — `WatchSync
    /// .planPreferencePairs` — and a settings change reloads every
    /// widget), the reminders that judge today, and this screen's own
    /// history, which re-grades.
    private func setBurnCorrection(_ kcal: Double) {
        SharedStore.setBurnCorrection(kcal: kcal)
        ...
        ToastCenter.shared.show(kcal == 0 ? "Burn correction removed ✓" : "Burn correction set ✓")
    }
    ...
    private func budgetCompositionSection(_ plan: CalorieBudget.Plan?) -> some View {
        Section {
            ...
            DisclosureGroup("How your budget is calculated") {
                ...
                // recipe — "we know that is the deficit, but how is it
                ...
                if !isMaintenance, let plan, let daysToTarget,
                   let current = planWeightLb, let target = targetWeightLb,
                   current > target {
                    Text("\(unit.fromLb(current - target), format: .number.precision(.fractionLength(1))) \(unit.symbol) over \(daysToTarget) days needs \(plan.requiredDailyDeficit, format: .number.precision(.fractionLength(0))) kcal/day. Your budget is your burn\(burnCorrectionKcal != 0 && model.averageBurnKcal != nil ? ", corrected," : "") minus that.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
                ...
                // A term in the recipe once one is accepted: the budget
                // above is cut from the average PLUS this, so leaving it
                // out would make the arithmetic uncheckable — the very
                // fault this disclosure exists to fix. Absent at 0, so a
                // screen with no correction is unchanged.
                if burnCorrectionKcal != 0, model.averageBurnKcal != nil {
                    LabeledContent("Burn correction") {
                        Text("\(signedKcal(burnCorrectionKcal)) kcal/day")
                            .monospacedDigit()
                    }
                    // Maintenance has no Progress section, so the one
                    // tap out lives here too.
                    if isMaintenance {
                        Button("Remove Correction", role: .destructive) {
                            setBurnCorrection(0)
                        }
                ...
                // `ObservedBurn` and its tests stayed in the kit, and
                // the cross-check RETURNED on 2026-09-22 — to Progress,
                // beside the predicted-vs-actual pair it explains, which
                // is the placement this note named. Still not HERE: the
                // recipe is the only thing in the explainer, and a
                // second burn figure under a heading promising one
                // calculation is what made it unreadable the first time.
                //
                ...
                // accepted: "Burned today" reading far above the Health
                // app's so-far figure now has no explanation anywhere, and
    ...
    /// carrying both ("182.4 lb  7-day average ›") made the reader work
    ...
    var body: some View {
        NavigationStack {
            ...
            // States the consequence BEFORE it is taken, re-grade
            // included — the one-way-in-your-head half of a reversible
            // action (PLAN-burn-correction, Hazards).
            .alert(
                proposedCorrection.map {
                    "\($0.kcalPerDay < 0 ? "Lower" : "Raise") every day's burn by \(abs($0.kcalPerDay).formatted(.number.precision(.fractionLength(0)))) kcal?"
                } ?? "",
                isPresented: Binding(
                    get: { proposedCorrection != nil },
                    set: { if !$0 { proposedCorrection = nil } }),
                presenting: proposedCorrection
            ) { suggestion in
                Button("Use \(signedKcal(suggestion.kcalPerDay)) kcal/day") {
                    setBurnCorrection(suggestion.kcalPerDay)
                }
                Button("Cancel", role: .cancel) {}
            } message: { suggestion in
                Text("Past days re-grade too, so badges and your streak can change.")
            }
    ...
    /// row on this screen reads "182.2 lb", and an editable one labelled
    ...
    private func approachingRows(basisLb: Double, remainingLb: Double) -> some View {
        ...
        // that anyone deciding "this target, then 10 lb more" a few days
        // early had no choice but to hand-edit — which re-stamped the
        // start and re-zeroed a bar with pounds behind it.
        keepGoingControls
    }
    ...
    private func save(continuing: Bool = false, decidedFromReached: Bool = false) {
        ...
        // target-changed re-stamp, so the bar reads N of M rather than
        ...
        // destination — one target reached, a lower one next — so it keeps its start
