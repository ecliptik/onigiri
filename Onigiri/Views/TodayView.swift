...
struct TodayView: View {
    ...
    /// Accepted or removed on Goal; the day re-derives from what is
    /// already loaded (`TodayModel.burnCorrectionChanged`).
    @AppStorage(SharedStore.burnCorrectionKcalKey, store: SharedStore.defaults)
    private var burnCorrectionSetting = 0.0
    ...
    var body: some View {
        ...
        .onChange(of: burnCorrectionSetting) { _, _ in
    ...
    private func plan(for goal: GoalSettings) -> CalorieBudget.Plan? {
        ...
        // Details showed hundreds of "kcal left" two rows above a
        // "kcal surplus" (2026-08-02). The model computes it once per refresh
    ...
    private var meterGrid: some View {
        Grid(horizontalSpacing: 12, verticalSpacing: 12) {
            GridRow {
                ...
                // with the deficit printed above it (July 27: the two raw
                // rows summed hundreds short of the burn the deficit was
                // cut from).
                MeterCell(label: "Resting", value: model.creditedRestingKcal) {
                    Image(systemName: "bed.double.fill").foregroundStyle(.indigo)
                }
                // The burn correction, when one is live and landed — its
                // own cell, because the three beside it have to add up to
                // the Burned flank and the correction is neither channel
                // (`plans/PLAN-burn-correction.md`). Absent at 0.
                if model.burnCorrectionKcal.rounded() != 0 {
                    MeterCell(label: "Correction", value: model.burnCorrectionKcal) {
                        Image(systemName: "scalemass.fill").foregroundStyle(.secondary)
                    }
                }
            }
        }
...
struct DailyGoalCard: View, Equatable {
    ...
    /// read "N of M kcal deficit — earned" on Today while the calendar
    /// left it blank: a few hundred kcal logged, under the threshold
    /// (the user, 2026-08-02).
        ...
        HStack(spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                HStack(spacing: 6) {
                    ...
                    // above "Eaten N of M" (65% eaten) and the
                ...
                if showsRemaining {
                    ...
                    // Name the threshold. A day showing a deficit well past
                    // its target and then quietly not
