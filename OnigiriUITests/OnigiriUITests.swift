...
final class OnigiriUITests: XCTestCase {
    ...
    /// 08-24) where it read as a derivation rather than a check.
    ///
    /// Needs `--seed-long-history`: the figure is SILENT under
    /// `ObservedBurn.minimumTrackedDays` (21) tracked days in the
    /// trailing 30, and the ordinary seed writes three. Without that
    /// flag this test would be the "probe that guards nothing" — an
    /// assertion about a row that could never have appeared.
    ///
    /// Asserts the PLACEMENT, not just the presence. Placement is the
    /// whole content of the decision: the number was on Goal before and
    /// was cut for sitting in the wrong section, so a test that only
    /// checked it existed would pass against the arrangement that was
    /// already rejected once.
    @MainActor
    func testGoalScaleBurnSitsWithTheGapItExplains() throws {
        let app = XCUIApplication()
        app.launchArguments = ["--seed-sample-data", "--seed-long-history"]
        XCUIDevice.shared.orientation = .portrait
        app.launch()
        skipOnboardingIfPresent(in: app)
        grantHealthAccess(in: app, timeout: 30)
        ...
        // A Form renders lazily, so these rows do not merely sit off
        // screen — they do not EXIST until scrolled to.
        _ = app.staticTexts["Current weight"].waitForExistence(timeout: 30)
        let scaleBurn = app.staticTexts["Burn, from the scale"]
        for _ in 0..<8 where !scaleBurn.exists { app.swipeUp() }
        XCTAssertTrue(
            scaleBurn.waitForExistence(timeout: 5),
            "28 seeded days clear ObservedBurn.minimumTrackedDays, so the "
                + "cross-check should be on Goal. If this is the only failure, "
                + "check --seed-long-history still reaches seedSampleData"
        )
        settle(scaleBurn)

        // BESIDE the pair it checks. "Last 30 days" carries the
        // predicted and on-scale figures the cross-check reconciles;
        // above it, or in another section, it reads as a second
        // measured burn — which is exactly why it was cut before.
        let lastThirty = app.staticTexts["Last 30 days"]
        XCTAssertTrue(lastThirty.exists, "the cross-check outlived the gap it explains")
        XCTAssertLessThan(
            lastThirty.frame.minY, scaleBurn.frame.minY,
            "the cross-check belongs under the pair it reconciles, not above it"
        )

        // And it must not have drifted into the recipe, which holds the
        // recipe and nothing else. The explainer is collapsed here, so
        // its own burn row is absent from the tree either way; this
        // pins the LABEL the explainer used, which must stay retired.
        XCTAssertFalse(
            app.staticTexts["Average burn, from data"].exists,
            "that label belongs to the explainer row cut on 2026-08-24"
        )

        // The value, not just the row: a label with a broken figure
        // beside it is the failure this whole screen keeps having.
        let value = app.staticTexts.matching(
            NSPredicate(format: "label CONTAINS 'kcal/day'")
        ).allElementsBoundByIndex.first {
            abs($0.frame.midY - scaleBurn.frame.midY) < 12
        }
        let text = try XCTUnwrap(value?.label, "no kcal/day figure on the cross-check row")
        let digits = Int(text.filter(\.isNumber)) ?? 0
        // The seeded cycle eats ~1,403 kcal/day against ~0.067 lb/day
        // off the scale, so the implied burn lands near 1,640. A wide
        // band: this asserts the arithmetic RAN, not the fixture.
        XCTAssertTrue(
            (900...3_500).contains(digits),
            "implied burn read \(text) — outside anything a body burns"
        )

        let shot = XCTAttachment(screenshot: XCUIScreen.main.screenshot())
        shot.name = "goal-scale-burn"
        shot.lifetime = .keepAlways
        add(shot)
    }

    /// The burn correction end to end (`plans/PLAN-burn-correction.md`):
    /// the offer states its basis, accepting re-grades history, Today's
    /// Details still ADDS UP with the correction as its own row, and
    /// removing it puts every figure back.
    @MainActor
    func testBurnCorrectionOfferAcceptAndClear() throws {
        let app = XCUIApplication()
        // The seed clears any correction an earlier run left behind.
        app.launchArguments = ["--seed-sample-data", "--seed-long-history"]
        XCUIDevice.shared.orientation = .portrait
        app.launch()
        skipOnboardingIfPresent(in: app)
        ...
        let netBefore = try detailsFigures(in: app).net
        XCTAssertFalse(app.staticTexts["Burn correction"].exists,
                       "no correction row before one is accepted")
        app.navigationBars["Details"].buttons.firstMatch.tap()

        // The offer: past the 21-day gate, beside the scale burn, with
        // its basis stated.
        switchTab(in: app, to: "Goal")
        _ = app.staticTexts["Current weight"].waitForExistence(timeout: 30)
        let offer = app.buttons["burnCorrectionOffer"]
        for _ in 0..<8 where !offer.exists { app.swipeUp() }
        XCTAssertTrue(offer.waitForExistence(timeout: 5),
                      "28 seeded days should earn an offer under Burn, from the scale")
        settle(offer)
        XCTAssertTrue(offer.label.hasPrefix("Correct Burn by −"), offer.label)
        let basis = app.staticTexts.matching(
            NSPredicate(format: "label BEGINSWITH 'From ' AND label CONTAINS 'tracked days since'")
        ).firstMatch
        XCTAssertTrue(basis.exists, "the offer must say what it was calibrated against")
        let deficitBefore = try totalDeficitKcal(in: app)

        offer.tap()
        let use = app.alerts.buttons.matching(NSPredicate(format: "label BEGINSWITH 'Use '")).firstMatch
        XCTAssertTrue(use.waitForExistence(timeout: 5), "accepting must go through the consequence alert")
        XCTAssertTrue(app.alerts.staticTexts.matching(
            NSPredicate(format: "label CONTAINS 'Past days re-grade'")).firstMatch.exists,
                      "the alert states the re-grade before it happens")
        use.tap()

        let status = app.descendants(matching: .any)["burnCorrectionStatus"]
        XCTAssertTrue(status.waitForExistence(timeout: 10), "accepted correction shows in force")
        XCTAssertFalse(offer.exists, "the offer yields to the correction in force")
        // History re-grades: the same tracked days bank less.
        let deficitAfter = try waitForChange(from: deficitBefore) { try self.totalDeficitKcal(in: app) }
        XCTAssertLessThan(deficitAfter, deficitBefore, "a lowered burn must lower Total deficit")
        let shotGoal = XCTAttachment(screenshot: XCUIScreen.main.screenshot())
        shotGoal.name = "burn-correction-goal"
        shotGoal.lifetime = .keepAlways
        add(shotGoal)

        // Today's Details: its own row, and the card still adds up.
        switchTab(in: app, to: "Today")
        let live = try detailsFigures(in: app)
        let correction = try XCTUnwrap(live.correction, "Details must print the correction as its own row")
        XCTAssertLessThan(correction, 0)
        XCTAssertEqual(live.active + live.resting + correction - live.intake, live.net, accuracy: 3,
                       "Active + Resting + Correction − Intake must equal Net")
        let shotToday = XCTAttachment(screenshot: XCUIScreen.main.screenshot())
        shotToday.name = "burn-correction-details"
        shotToday.lifetime = .keepAlways
        add(shotToday)
        app.navigationBars["Details"].buttons.firstMatch.tap()

        // One tap out, and every figure returns.
        switchTab(in: app, to: "Goal")
        let remove = app.buttons["Remove Correction"]
        for _ in 0..<8 where !remove.exists { app.swipeUp() }
        XCTAssertTrue(remove.waitForExistence(timeout: 5))
        settle(remove)
        remove.tap()
        XCTAssertTrue(offer.waitForExistence(timeout: 10), "clearing brings the offer back")
        let deficitCleared = try waitForChange(from: deficitAfter) { try self.totalDeficitKcal(in: app) }
        XCTAssertEqual(deficitCleared, deficitBefore, "clearing must restore Total deficit exactly")

        switchTab(in: app, to: "Today")
        let cleared = try detailsFigures(in: app)
        XCTAssertNil(cleared.correction)
        XCTAssertEqual(cleared.net, netBefore, accuracy: 1, "clearing must restore today's Net")
        app.navigationBars["Details"].buttons.firstMatch.tap()
    }

    /// The first signed integer in a label ("−300 kcal", "1,234 / 500").
    private func leadingKcal(_ label: String) -> Double? {
        guard let match = label.firstMatch(of: /([−-])?(\d[\d,]*)/) else { return nil }
        let value = Double(match.2.replacingOccurrences(of: ",", with: "")) ?? 0
        return match.1 == nil ? value : -value
    }

    /// The value printed on the same row as a label (LabeledContent).
    @MainActor
    private func valueOnRow(_ title: String, in app: XCUIApplication) throws -> String? {
        let label = app.staticTexts[title]
        guard label.waitForExistence(timeout: 3) else { return nil }
        return app.staticTexts.matching(NSPredicate(format: "label CONTAINS 'kcal'"))
            .allElementsBoundByIndex
            .filter { abs($0.frame.minY - label.frame.minY) < 12 && $0.frame.minX > label.frame.maxX }
            .first?.label
    }

    private struct DetailsFigures {
        var intake, active, resting, net: Double
        var correction: Double?
    }

    /// Opens Today's Details and reads the energy rows off it.
    @MainActor
    private func detailsFigures(in app: XCUIApplication) throws -> DetailsFigures {
        detailsLink(in: app).tap()
        XCTAssertTrue(app.navigationBars["Details"].waitForExistence(timeout: 10))
        _ = app.staticTexts["Resting burn"].waitForExistence(timeout: 10)
        func read(_ title: String) throws -> Double {
            let text = try XCTUnwrap(try valueOnRow(title, in: app), "no value on \(title)")
            return try XCTUnwrap(leadingKcal(text), "unreadable \(title): \(text)")
        }
        let intakeTitle = ["Eaten", "Intake", "Consumed"].first { app.staticTexts[$0].exists } ?? "Eaten"
        let netText = try XCTUnwrap(try valueOnRow("Net", in: app))
        let netMagnitude = try XCTUnwrap(leadingKcal(netText))
        return DetailsFigures(
            intake: try read(intakeTitle),
            active: try read("Active burn"),
            resting: try read("Resting burn"),
            net: netText.contains("excess") ? -netMagnitude : netMagnitude,
            correction: app.staticTexts["Burn correction"].exists ? try read("Burn correction") : nil)
    }

    /// Goal's "Total deficit" in kcal (scrolled to if needed).
    @MainActor
    private func totalDeficitKcal(in app: XCUIApplication) throws -> Double {
        // The only figure on Goal reading "N kcal ≈ M lb".
        let figure = app.staticTexts.matching(NSPredicate(format: "label CONTAINS 'kcal ≈'")).firstMatch
        for _ in 0..<8 where !figure.exists { app.swipeUp() }
        for _ in 0..<8 where !figure.exists { app.swipeDown() }
        XCTAssertTrue(figure.waitForExistence(timeout: 5), "no Total deficit figure")
        return try XCTUnwrap(leadingKcal(figure.label), figure.label)
    }

    /// Polls a figure until it moves off `old` — a reload is async.
    @MainActor
    private func waitForChange(
        from old: Double, timeout: TimeInterval = 10, _ read: () throws -> Double
    ) throws -> Double {
        let deadline = Date().addingTimeInterval(timeout)
        var value = try read()
        while value == old, Date() < deadline {
    ...
    /// The row `+`'s long press, on both hosts. Each type's long press is
    /// the other's tap — a FOOD logs its default portion outright, a MEAL
    /// opens the portion sheet — and nothing in the suite covered either
    /// half, which is how the food half could stop firing unnoticed.
    @MainActor
    func testRowPlusLongPressLogsDefaultPortion() throws {
        let app = XCUIApplication()
        XCUIDevice.shared.orientation = .portrait
        app.launchArguments = ["--seed-sample-data"]
        app.launch()
        ...
        // The seed has to LAND first: while it writes, the Add tab is
        // present but unhittable, and a leg that starts there fails on
        // the tab rather than on the gesture. Waited for, not asserted
        // — the precondition is the TAB being reachable, and how long
        // months of HealthKit samples take to land on a freshly erased
        // simulator is not this test's subject.
        ...
        // Leg 1 — the Log sheet, FIRST and from a screen with no
        // toast standing over the tab bar: a toast left up by an
        // earlier leg swallows the Add tap, the Log sheet never
        // presents, and the row + found afterwards is the FOODS row —
        // a leg that passes having tested the other host.
        switchTab(in: app, to: "Add")
        if !app.navigationBars["Log"].waitForExistence(timeout: 10) {
            switchTab(in: app, to: "Add")
        }
        ...
        let sheetPlus = app.buttons["Log Protein shake"].firstMatch
        XCTAssertTrue(sheetPlus.waitForExistence(timeout: 10), "Log sheet row +")
        // Prove it is the SHEET's row, not the Foods list showing
        // through a sheet that never presented.
        XCTAssertTrue(app.navigationBars["Log"].exists, "still on the Log sheet")
        sheetPlus.press(forDuration: 1.0)
        XCTAssertTrue(
            app.staticTexts.matching(
                NSPredicate(format: "label BEGINSWITH 'Logged Protein shake'")
            ).firstMatch.waitForExistence(timeout: 8),
            "A food's + should log its default portion on a long press (Log sheet)"
        )

        // Leg 2 — the same gesture on the Foods list.
        if app.navigationBars["Log"].exists {
            app.buttons["Cancel"].firstMatch.tap()
        }
        switchTab(in: app, to: "Foods")
        let foodsPlus = app.buttons["Log Protein shake"].firstMatch
        XCTAssertTrue(foodsPlus.waitForExistence(timeout: 15), "Foods row +")
        foodsPlus.press(forDuration: 1.0)
        XCTAssertTrue(
            app.staticTexts.matching(
                NSPredicate(format: "label BEGINSWITH 'Logged Protein shake'")
            ).firstMatch.waitForExistence(timeout: 8),
            "A food's + should log its default portion on a long press (Foods)"
        )
        // …and must NOT have opened the portion sheet, which is its TAP.
    ...
    func testMenuPickerLogsSeveralItems() throws {
        ...
        /// the confirm — a copy of the picked row would not.
        func edit(_ name: String, adding token: String) {
            // By the button's own name: the item row was the way in for
            // a day and read as inert (the user, 2026-09-21), so the
            // affordance is a named control now and the test asks for
            // it by name.
            let editButton = app.buttons["Edit Item"]
            XCTAssertTrue(editButton.waitForExistence(timeout: 10),
                          "The confirm should carry an explicit Edit Item button")
            editButton.tap()
            let editor = app.navigationBars["Edit Item"]
            XCTAssertTrue(editor.waitForExistence(timeout: 10), "The item editor should push")
            let field = app.textFields["Name"]
            XCTAssertTrue(field.waitForExistence(timeout: 5), "The editor's Name field")
            field.tap()
            ...
            // (2026-09-21), so it is plain text again.
            XCTAssertTrue(
                app.staticTexts.matching(NSPredicate(format: "label CONTAINS %@", token))
                    .firstMatch.waitForExistence(timeout: 5),
                "The confirm should show what was typed in the editor")
            // NOT asserted here: that the pop doesn't re-open "Where is
            // this menu from?". A covered NavigationStack root re-runs
            // its `.task`, which is why `MenuPickerFlow` guards its
            // setup — but this sample NAMES its cafe, so the prompt is
            // never raised in this run and the check would pass over a
            // build that had the bug. It is verified from frames in the
            // share extension instead (`plans/PLAN-multi-item-import.md`).
        }

        /// Pick a row, confirm it, and come back. Waits for the picker's
        /// own title to return, which is the loop closing.
        func logItem(_ name: String, editing token: String? = nil, expectFinalNote note: String) {
        ...
        // Back out of a confirm before anything is logged. Discarding
        // the pick clears the item the confirm is still drawing, which
        // a force-unwrapping `Binding($chosen)` traps the process on —
        // no message, no crash banner, the sheet simply disappears
        // (2026-09-20). If that returns, this assertion fails because
        // the app is gone.
        let backOut = app.buttons.matching(
            NSPredicate(format: "label CONTAINS 'Sample Bowl'")
        ).firstMatch
        ...
        XCTAssertTrue(picker.waitForExistence(timeout: 10),
                      "Back should return to the list with the app still running")

        // "Zqx" is a token nothing else on screen can contain, so
        // finding it later proves the typed name travelled.
        logItem("Sample Bowl", editing: "Zqx", expectFinalNote: "Zqx")
        ...
        expandMealSections(in: app)
        // The source is folded in when the row is picked, so the entries
        // read "Sample Bowl (Sample Cafe)" — and the first one carries
        // the editor's token, which is the proof that what was TYPED is
        // what Health was told, rather than what was read.
        for name in ["Zqx", "Sample Fries"] {
