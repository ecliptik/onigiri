...
final class OnigiriUITests: XCTestCase {
    ...
    /// The Today card's + deep link opens the app's Log sheet. The +
    /// is a Link (openURL), NOT an AppIntent, so it dispatches on the
    /// simulator where interactive widget intents (linkd) don't. Opt in
    /// via TODAY_DEEPLINK=1.
    @MainActor
    func testTodayCardDeepLink() throws {
        guard ProcessInfo.processInfo.environment["TODAY_DEEPLINK"] == "1" else {
            throw XCTSkip("Set TODAY_DEEPLINK=1 to run the Today-card deep-link test")
        }
        let app = XCUIApplication()
        XCUIDevice.shared.orientation = .portrait
        app.launchArguments = ["--seed-sample-data"]
        ...
        springboard.buttons["Log food or meal"].firstMatch.tap()
        XCTAssertTrue(app.wait(for: .runningForeground, timeout: 10), "App opened from + deep link")
        XCTAssertTrue(app.navigationBars["Log"].waitForExistence(timeout: 10), "Log sheet after + deep link")
        attachShot(named: "deeplink-log-sheet", settle: 2)
    }

    /// 2.1 M2 QA — ON DEVICE: installs the LARGE Today card (shooting
    /// every family's gallery page on the way), drives its buttons —
    /// water in place, ‹ › paging bounded at today — and the + deep link
    /// into the app's Log sheet for the shown day; then adds the MEDIUM
    /// variant for the family shot. The AppIntent buttons (water, paging)
    /// need a device: the simulator's linkd doesn't index widget-intent
    /// metadata, so those taps no-op there (the shipped water button
    /// fails identically) — the + deep link is a Link/openURL and works
    /// on the sim (see testTodayCardDeepLink). Opt in via TODAY_WIDGET_QA=1.
    @MainActor
    func testTodayCardWidgetQA() throws {
        guard ProcessInfo.processInfo.environment["TODAY_WIDGET_QA"] == "1" else {
            throw XCTSkip("Set TODAY_WIDGET_QA=1 to run the Today-card widget QA")
        }
        let app = XCUIApplication()
        XCUIDevice.shared.orientation = .portrait
        app.launchArguments = ["--seed-sample-data"]
        ...
        // The pager orders families small→large; the LAST page matching
        // our description is the large card.
        func addTodayCard(page: String) {
            springboard.activate()
            XCUIDevice.shared.press(.home)
            Thread.sleep(forTimeInterval: 1)
            XCUIDevice.shared.press(.home)
            ...
            XCTAssertTrue(todayPage.exists, "Today card page in the family pager")
            attachShot(named: "gallery-\(page)-first-family", settle: 1)
            if page == "large" {
                // Advance to the last matching page (the large family).
                while true {
                    if pager.exists { pager.swipeLeft() } else { springboard.swipeLeft() }
                    Thread.sleep(forTimeInterval: 0.6)
                    if todayPage.exists {
                        attachShot(named: "gallery-\(page)-next-family", settle: 0.4)
                    } else {
                        if pager.exists { pager.swipeRight() } else { springboard.swipeRight() }
                        Thread.sleep(forTimeInterval: 0.6)
                        break
                    }
                }
        ...
        addTodayCard(page: "large")
        attachShot(named: "widget-today-initial", settle: 1)

        // Page back two days, then forward to the today bound.
        springboard.buttons["Previous day"].firstMatch.tap()
        Thread.sleep(forTimeInterval: 6)
        attachShot(named: "widget-yesterday", settle: 1)
        springboard.buttons["Previous day"].firstMatch.tap()
        Thread.sleep(forTimeInterval: 6)
        attachShot(named: "widget-two-days-back", settle: 1)
        springboard.buttons["Next day"].firstMatch.tap()
        Thread.sleep(forTimeInterval: 6)
        springboard.buttons["Next day"].firstMatch.tap()
        Thread.sleep(forTimeInterval: 6)
        attachShot(named: "widget-back-to-today", settle: 1)

        // Water in place: a serving onto the water pill, no app launch.
        springboard.buttons["Log water"].firstMatch.tap()
        Thread.sleep(forTimeInterval: 8)
        attachShot(named: "widget-after-water", settle: 1)

        // Browse to yesterday and take + into the app: the Log sheet,
        // for the shown day.
        springboard.buttons["Previous day"].firstMatch.tap()
        Thread.sleep(forTimeInterval: 6)
        springboard.buttons["Log food or meal"].firstMatch.tap()
        XCTAssertTrue(app.wait(for: .runningForeground, timeout: 10), "App opened from + deep link")
        XCTAssertTrue(app.navigationBars["Log"].waitForExistence(timeout: 10), "Log sheet after + deep link")
        attachShot(named: "app-log-sheet-deep-link", settle: 2)
        let doneButton = app.buttons["Done"].firstMatch
        XCTAssertTrue(doneButton.waitForExistence(timeout: 5), "Log sheet Done")
        doneButton.tap()
        let title = app.buttons["dayTitleButton"]
        XCTAssertTrue(title.waitForExistence(timeout: 5), "Day title after sheet dismiss")
        XCTAssertTrue(title.label.contains("Yesterday"), "Deep link browsed to the shown day: \(title.label)")
        attachShot(named: "app-yesterday-after-dismiss", settle: 1)

        // The medium family, for the side-by-side QA shot.
        addTodayCard(page: "medium")
        attachShot(named: "home-both-families", settle: 1)
    }
