...
struct DaySnapshot: Codable {
    ...
    /// The burn correction as applied today — the third row of that
    /// split, so it still adds up (`plans/PLAN-burn-correction.md`). nil
    /// for a cached snapshot from before it existed.
    var creditedCorrectionKcal: Double?
    ...
    var planState: DailyPlanLoader.State {
        ...
        return DailyPlanLoader.State(
            ...
            creditedCorrectionKcal: creditedCorrectionKcal
        )
    }
    ...
    var newDay: DaySnapshot {
        DaySnapshot(
            ...
            // Plan-derived: the accepted correction carries into the new
            // day, as the budget above does.
            creditedCorrectionKcal: creditedCorrectionKcal,
...
extension DaySnapshot {
    init(from decoder: Decoder) throws {
        ...
        creditedCorrectionKcal = try c.decodeIfPresent(Double.self, forKey: .creditedCorrectionKcal)
...
enum SnapshotLoader {
    ...
    static func load() async -> Load {
        ...
        let snapshot = DaySnapshot(
            ...
            creditedCorrectionKcal: state.creditedCorrectionKcal,
        ...
        // none — it made the next observer fire see a burn jump that
