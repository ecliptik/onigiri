...
struct TodayCardProvider: TimelineProvider {
    ...
    func getSnapshot(in context: Context, completion: @escaping (TodayCardEntry) -> Void) {
        ...
        Task { @MainActor in
            completion(TodayCardEntry(date: .now, snapshot: await SnapshotLoader.load().snapshot))
        }
    }
    ...
    private var creditedSplit: some View {
        ...
        if let active = plan.creditedActiveKcal, let resting = plan.creditedRestingKcal {
            VStack(alignment: .leading, spacing: 8) {
                ...
                // Its own row, as on Today's Details — the split adds up
                // to the burn the budget was cut from only with it.
                if let correction = plan.creditedCorrectionKcal, correction.rounded() != 0 {
                    creditedRow(
                        "Correction", systemImage: "scalemass.fill", tint: .secondary,
                        credited: correction, measured: correction, measuredCaption: "")
                }
            }
        }
    }
