...
public enum CalorieBudget {
    ...
    /// that never happened ("kcal left" on a day that ended over
    /// break-even, 2026-07-30), and an in-progress one kept the promise
    /// right up to bedtime (hundreds of "kcal left" beside a "kcal
    /// surplus", 2026-08-02). Crediting resting up front is what makes the forecast
    ...
    public static func completedDayPlan(
        ...
        return Plan(
            ...
            // 10 lb lower against a stale date wanted a deficit hundreds
            // of kcal/day larger than the day's whole burn, so the budget
            // went negative and Today read "+N kcal over" at 9am with
            // nothing logged
    ...
    /// still left the two screens hundreds of kcal apart at lunchtime with the
...
public extension CalorieBudget {
    ...
    /// directly above a goal card reading "≈ N kcal over budget" — one
