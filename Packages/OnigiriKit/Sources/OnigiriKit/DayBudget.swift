...
/// could show hundreds of "kcal left" two rows above a "kcal surplus"
/// (2026-08-02).
///
...
public enum DayBudget {
    ...
    ///
    /// `burnCorrectionKcal` is the person-accepted offset
    /// (`BurnCorrection`, `plans/PLAN-burn-correction.md`) — EXPLICIT, never
    /// read from `SharedStore` in here, the same way the estimate arrives:
    /// every caller says which correction it judged by, so a test can pin
    /// it and a surface that forgot it fails to compile rather than
    /// quietly judging by another. 0 is off and returns exactly what this
    /// returned before the parameter existed.
    public static func dayBurn(
        ...
        BurnCorrection.apply(
            toBurnKcal: uncorrectedDayBurn(
                activeKcal: activeKcal, restingKcal: restingKcal,
                estimatedRestingKcal: estimatedRestingKcal),
            estimatedRestingKcal: estimatedRestingKcal,
            correctionKcal: burnCorrectionKcal)
    }

    /// The day's burn as Health measured it — resting floored by the
    /// estimate, plus active — before any correction. What
    /// `TodayBurnFloor` ratchets (it guards a MEASUREMENT against Health
    /// revising down), and what the Active/Resting rows split, since the
    /// correction prints as a row of its own.
    public static func uncorrectedDayBurn(
        activeKcal: Double,
        restingKcal: Double,
        estimatedRestingKcal: Double?
    ) -> Double {
    ...
    /// budget and Net were cut from, by a few kcal the screen never
    /// accounted for (the user, 2026-08-24). `TodayBurnFloor`
