...
public struct GoalTrendStats: Equatable, Sendable {
    ...
    /// The burn correction the scale proposes, over ALL completed tracked
    /// days (`BurnCorrection.suggest`). An OFFER for Goal to show beside
    /// `observedBurnKcal` — nothing reads it into a budget; only a tap
    /// stores a correction.
    public let burnCorrectionSuggestion: BurnCorrection.Suggestion?
    ...
    public init(
        ...
        observedBurnKcal: Double? = nil,
        burnCorrectionSuggestion: BurnCorrection.Suggestion? = nil,
        ...
        self.burnCorrectionSuggestion = burnCorrectionSuggestion
    ...
    public static func derive(
        ...
        // What the scale says the burn really was. Reported, never
        // planned from: see `ObservedBurn` for the five causes this
        // single number cannot tell apart, and for why feeding it back
        // into `dayBurn` would resurrect the deleted trailing-average
        // substitution.
        //
        // No companion "measured" figure rides with it. One was tried
        // and removed the same day: a mean over these tracked days is
        // the honest basis for the subtraction but is NOT the burn the
        // screen shows, so the delta contradicted the "Average daily
        // burn" row three lines above it (2026-08-18). The view puts the
        // two burns near each other and lets the reader compare.
        ...
        return GoalTrendStats(
            ...
            observedBurnKcal: observedBurn,
            burnCorrectionSuggestion: BurnCorrection.suggest(
                dailyTotals: dailyTotals, weightHistory: weightHistory,
                untrackedBelowKcal: untrackedBelowKcal, calendar: calendar, now: now),
