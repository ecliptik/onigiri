...
public final class HealthKitService {
    ...
    ///
    /// Why (2026-08-04, the user): a day's intake read at well under half
    /// its real total. The log listed all three entries; the totals above
    /// them dropped exactly the sandwich logged on the WATCH. Apple Health
    /// showed the same undercount.
    ///
    ...
    public func dailyEnergyTotals(
        days: Int = 92, now: Date = .now, burnCorrectionKcal: Double
    ) async throws -> [DayEnergyTotals] {
        let calendar = Calendar.current
        guard let start = calendar.date(byAdding: .day, value: -days, to: calendar.startOfDay(for: now)) else {
            return []
        }
        return try await dailyEnergyTotals(from: start, to: now, burnCorrectionKcal: burnCorrectionKcal)
    }
    ...
    /// whatever active energy was actually earned — less the burn
    /// correction, when one has been accepted. That is how a correction
    /// re-grades history (`plans/PLAN-burn-correction.md`, decision 1):
    /// every verdict this feeds reads one corrected burn. The caller
    /// passes it (`SharedStore.burnCorrectionKcal`), explicitly.
    public func dailyEnergyTotals(
        from start: Date, to end: Date, burnCorrectionKcal: Double
    ) async throws -> [DayEnergyTotals] {
        ...
        // an undercount — at well under half its real total it fell under
    ...
    /// boundary instead. That was the Today-vs-calendar burn gap (tens
    /// of kcal for one day, 2026-07-30). Intake, sodium and water
    ...
    /// - Parameter longHistory: extend the seeded past from three days
    ///   to `longSeedHistoryDays`. OFF by default, and the default must
    ///   stay byte-identical to the three-day seed —
    ///   `testSeedGrantAndLogFlow` asserts exact totals against it.
    ///
    ///   It exists for ONE row: Goal's "Burn, from the scale"
    ///   (`ObservedBurn`) says nothing under
    ///   `ObservedBurn.minimumTrackedDays` tracked days in the trailing
    ///   30, and three can never reach it — so without this the row was
    ///   untestable on a simulator, and an "is it absent?" probe against
    ///   a row that could never appear would have passed forever while
    ///   guarding nothing.
    ///
    ///   Needs no guard of its own and no undo: `clearSeededSamples`
    ///   above deletes everything the app wrote before every seed, so a
    ///   later run WITHOUT the flag takes the long history away by
    ///   construction (simulator only, like that reset).
    public func seedSampleData(now: Date = .now, longHistory: Bool = false) async throws {
        ...
        // The three days CYCLE when a long history is asked for, rather
        // than a fourth shape being invented: every day in
        // `seededHistory` is already tracked and comfortably in deficit,
        // which is what the calendar's earned badges and this seed's
        // whole point depend on, and cycling keeps the day-to-day
        // variety the site's day-swipe clip needs.
        let history = longHistory
            ? (0..<Self.longSeedHistoryDays).map {
                Self.seededHistory[$0 % Self.seededHistory.count]
            }
            : Self.seededHistory
    ...
    /// How far back `longHistory` seeds. Comfortably past
    /// `ObservedBurn.minimumTrackedDays` so the row is not sitting on
    /// its own threshold, and inside the 30 days of weigh-ins seeded
    /// above — the cross-check needs BOTH a scale rate and the tracked
    /// days, and a history longer than the weigh-ins would add neither.
    static let longSeedHistoryDays = 28
