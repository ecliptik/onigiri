...
struct DailyPlanLoaderTests {
    ...
    @Test func restingIsCreditedUpFrontAtBreakfast() throws {
        let state = DailyPlanLoader.makeState(
            goal: Self.maintenanceGoal(),
            summary: Self.summary(intake: 400, active: 50, resting: 600),
            estimatedRestingKcal: 1_800, healthWeightLb: nil,
            calendar: Self.cal, now: Self.now
        )
        // 50 earned + the full 1,800 baseline, not 650.
        #expect(try #require(state.dailyBudgetKcal) == 1_850)
        #expect(try #require(state.remainingKcal) == 1_450)
    }
    ...
    @Test func unwornDayKeepsTheBaselineAndEarnsNoActivity() throws {
        let worn = DailyPlanLoader.makeState(
            goal: Self.maintenanceGoal(),
            summary: Self.summary(intake: 0, active: 500, resting: 1_800),
            estimatedRestingKcal: 1_800, healthWeightLb: nil,
            calendar: Self.cal, now: Self.now
        )
        ...
        #expect(try #require(worn.dailyBudgetKcal) == 2_300)
        #expect(try #require(unworn.dailyBudgetKcal) == 1_800)
    }
