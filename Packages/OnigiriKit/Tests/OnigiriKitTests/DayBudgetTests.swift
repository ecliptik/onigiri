...
struct DayBudgetTests {
    ...
    @Test func restingIsCreditedUpFront() {
        // 8am: only 600 of the day's ~1,800 resting has accrued.
        #expect(DayBudget.dayBurn(
            activeKcal: 50, restingKcal: 600, estimatedRestingKcal: 1_800, burnCorrectionKcal: 0) == 1_850)
        // Same day at bedtime: measured has caught up, answer is stable
        // in the only way that matters — active is what moved it.
        #expect(DayBudget.dayBurn(
            activeKcal: 300, restingKcal: 1_800, estimatedRestingKcal: 1_800, burnCorrectionKcal: 0) == 2_100)
    }
    ...
    @Test func measuredRestingAboveTheEstimateWins() {
        #expect(DayBudget.dayBurn(
            activeKcal: 0, restingKcal: 2_000, estimatedRestingKcal: 1_800, burnCorrectionKcal: 0) == 2_000)
    }
    ...
    @Test func withoutBodyMetricsMeasuredRestingStandsAlone() {
        #expect(DayBudget.dayBurn(
            activeKcal: 300, restingKcal: 1_500, estimatedRestingKcal: nil, burnCorrectionKcal: 0) == 1_800)
    }
    ...
    @Test func activeEnergyIsEarnedNotBackfilled() {
        let worn = DayBudget.dayBurn(
            activeKcal: 300, restingKcal: 1_800, estimatedRestingKcal: 1_800, burnCorrectionKcal: 0)
        let watchInADrawer = DayBudget.dayBurn(
            activeKcal: 0, restingKcal: 1_800, estimatedRestingKcal: 1_800, burnCorrectionKcal: 0)
        #expect(worn - watchInADrawer == 300)
        // Resting still survives the drawer — only activity is at stake.
        #expect(watchInADrawer == 1_800)
    }
    ...
    /// by hand (the user, 2026-08-24: the two rows falling a few kcal short
    /// of the burn the budget was built on).
    @Test func theCreditedHalvesSumToTheBurnTheBudgetUsed() {
        let credited = DayBudget.creditedActive(
            dayBurnKcal: 2_420, creditedRestingKcal: 1_900, measuredActiveKcal: 500)
        #expect(credited == 520)
        #expect(credited + 1_900 == 2_420)
    }
    ...
    @Test func withoutARatchetCreditedActiveIsJustMeasured() {
        ...
        #expect(DayBudget.creditedActive(
            dayBurnKcal: dayBurn, creditedRestingKcal: 1_800,
            measuredActiveKcal: 300) == 300)
    }
    ...
    @Test func theRestingFloorDoesNotBecomeActive() {
        let dayBurn = DayBudget.dayBurn(
            activeKcal: 50, restingKcal: 600, estimatedRestingKcal: 1_800, burnCorrectionKcal: 0)
        #expect(DayBudget.creditedActive(
            dayBurnKcal: dayBurn, creditedRestingKcal: 1_800,
    ...
    @Test func aStaleDayBurnNeverUndercutsTheMeasurement() {
        #expect(DayBudget.creditedActive(
            dayBurnKcal: 0, creditedRestingKcal: 1_800, measuredActiveKcal: 120) == 120)
    }
    ...
    @Test func onlyAGapWorthExplainingPrintsTheMeasurement() {
        let ratchetNoise = DayBudget.creditedActive(
            dayBurnKcal: 2_420, creditedRestingKcal: 1_900, measuredActiveKcal: 500)
        #expect(ratchetNoise - 500 < DayBudget.creditNoteThresholdKcal)
        ...
        let realRevision = DayBudget.creditedActive(
            dayBurnKcal: 2_600, creditedRestingKcal: 1_900, measuredActiveKcal: 500)
        #expect(realRevision - 500 >= DayBudget.creditNoteThresholdKcal)
    ...
    @Test func netReadsTheBudgetsOwnBurn() {
        ...
        let intake = 800.0
        #expect(budget - intake == 700)
        #expect(DayBudget.deficit(intakeKcal: intake, dayBurnKcal: dayBurn) == 1_050)
        // The raw measured balance is what used to contradict it.
        let raw = DailyEnergySummary(
            intakeKcal: intake, activeBurnKcal: 50, restingBurnKcal: 600,
            sodiumMg: 0, waterOz: 0)
        #expect(raw.balanceKcal == 150) // a "surplus", beside 700 kcal left
    }
    ...
    @Test func pastTheBudgetSaysOverBudget() {
        let over = CalorieBudget.remainingHeadline(-250)
        #expect(over.value == 250)
