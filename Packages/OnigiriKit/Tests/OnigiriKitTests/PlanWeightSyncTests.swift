...
/// the screen — ~170 kcal apart, ~1.2 lb of basis at 3500/24 kcal per
/// pound (2026-08-08).
///
...
struct PlanWeightSyncTests {
    ...
    @Test func thePlanWeightCacheRoundTripsWithItsDay() {
        ...
        let now = Date.now
        HealthKitService.cachePlanWeight(182.4, now: now)
        let cached = HealthKitService.cachedPlanWeightLb()
        #expect(cached?.lb == 182.4)
    ...
    @Test func aWeightThatStaysGoneStopsBeingPushed() {
        ...
        HealthKitService.cachePlanWeight(182.4, now: staleDay)
    ...
    @Test func theBasisAlwaysRidesWithItsResolvedValue() {
        ...
        // First of the plan pairs; the burn correction's two keys follow
        // it (`BurnCorrectionStorageTests`).
    ...
    @Test func twoWeightsForOneDayIsAWholeBudgetApart() {
        ...
        func deficit(from weightLb: Double) -> Double {
            CalorieBudget.requiredDailyDeficit(
                currentWeightLb: weightLb, targetWeightLb: 180,
        ...
        let apart = deficit(from: 183.4) - deficit(from: 182.2)
