...
struct WeightBasisTests {
    ...
    @Test func aDayWithMorningAndEveningKeepsTheMorning() {
        let points = [at(daysAgo: 1, hour: 7, 182.2), at(daysAgo: 1, hour: 21, 184.8)]
        let lows = WeightTrend.dailyLows(points, calendar: calendar)
        #expect(lows.count == 1)
        #expect(lows[0].weightLb == 182.2)
    }

    @Test func theKeptPointCarriesItsOwnTimestamp() {
        // Not the day's start — windowing has to stay exact.
        let morning = at(daysAgo: 1, hour: 7, 182.2)
        let lows = WeightTrend.dailyLows([morning, at(daysAgo: 1, hour: 21, 184.8)], calendar: calendar)
    ...
    @Test func anEveningOnlyDayStillContributes() {
        ...
        let lows = WeightTrend.dailyLows([at(daysAgo: 2, hour: 22, 185.1)], calendar: calendar)
        #expect(lows.count == 1)
        #expect(lows[0].weightLb == 185.1)
    }
    ...
    @Test func daysAreCollapsedInTheGivenCalendar() {
        ...
        let points = [at(daysAgo: 2, hour: 23, 185.0), at(daysAgo: 1, hour: 7, 182.0)]
        #expect(WeightTrend.dailyLows(points, calendar: calendar).count == 2)
    }

    @Test func lowsComeBackDateAscending() {
        let points = [at(daysAgo: 1, hour: 7, 182.0), at(daysAgo: 5, hour: 7, 185.0),
                      at(daysAgo: 3, hour: 7, 184.0)]
        let lows = WeightTrend.dailyLows(points, calendar: calendar)
        #expect(lows.map(\.weightLb) == [185.0, 184.0, 182.0])
    }
    ...
    @Test func eveningWeighInsDoNotMoveTheBasis() {
        let mornings = (1...5).map { at(daysAgo: $0, hour: 7, 182.0 + Double($0) * 0.1) }
        let withEvenings = mornings + [at(daysAgo: 2, hour: 21, 185.0),
                                       at(daysAgo: 4, hour: 22, 185.4)]
    ...
    @Test func theBasisIsTheMeanOfDailyLows() {
        let points = [at(daysAgo: 1, hour: 7, 182.0), at(daysAgo: 2, hour: 7, 184.0),
                      at(daysAgo: 3, hour: 7, 186.0)]
    ...
    @Test func theWindowIsDaysNotReadings() {
        let inside = [at(daysAgo: 1, hour: 7, 182.0), at(daysAgo: 2, hour: 7, 182.0)]
        let old = [at(daysAgo: 20, hour: 7, 200.0), at(daysAgo: 30, hour: 7, 210.0)]
    ...
    @Test func aGapLongerThanTheWindowYieldsNil() {
        ...
        let stale = [at(daysAgo: 30, hour: 7, 200.0)]
    ...
    @Test func aSingleRecentReadingIsItsOwnBasis() {
        let one = [at(daysAgo: 1, hour: 7, 182.2)]
        #expect(WeightTrend.targetBasisLb(one, now: now, calendar: calendar) == 182.2)
    }

    @Test func futureDatedReadingsAreIgnored() {
        // A watch/phone clock skew must not let tomorrow into the window.
        let points = [at(daysAgo: 1, hour: 7, 182.0), at(daysAgo: -2, hour: 7, 160.0)]
        #expect(WeightTrend.targetBasisLb(points, now: now, calendar: calendar) == 182.0)
    }
    ...
    @Test func lastWeighInBasisIgnoresHistory() {
        let history = [at(daysAgo: 1, hour: 7, 182.0), at(daysAgo: 2, hour: 7, 186.0)]
        let resolved = WeightTrend.basisLb(
            .lastWeighIn, history: history, latestLb: 182.2, now: now, calendar: calendar)
        #expect(resolved == 182.2)
    }

    @Test func smoothedBasisFallsBackToLatestWhenUncomputable() {
        let resolved = WeightTrend.basisLb(
            .sevenDayAverage, history: [], latestLb: 182.2, now: now, calendar: calendar)
        #expect(resolved == 182.2)
    ...
    @Test func theProtocolDefaultFallsBackToTheRawLatest() async {
        #expect(await BareHealth(latest: 182.2).targetBasisWeightLb() == 182.2)
        #expect(await BareHealth(latest: nil).targetBasisWeightLb() == nil)
    }

    // MARK: - The observed case, as a fixture

    /// 2026-08-08: a 1.2 lb overnight drop, 24 days out, moved the
    /// deficit ~496 → ~321 and the budget up by the same ~175 (synthetic
    /// weights; the shape is the one that was reported).
    @Test func theObservedMorningJump() {
        let before = CalorieBudget.requiredDailyDeficit(
            currentWeightLb: 183.4, targetWeightLb: 180, daysRemaining: 24)
        let after = CalorieBudget.requiredDailyDeficit(
            currentWeightLb: 182.2, targetWeightLb: 180, daysRemaining: 24)
    ...
    @Test func sensitivityGrowsAsTheTargetDateApproaches() {
        func perPound(days: Int) -> Double {
            CalorieBudget.requiredDailyDeficit(currentWeightLb: 181, targetWeightLb: 180, daysRemaining: days)
        }
