...
- **Barcode, label & menu scanning** — one camera, no mode to pick: it reads a nutrition-facts label, turns a photographed menu board into a list, or (with AI) identifies food straight from a photo, with barcode scans and food search filled in from OpenFoodFacts and USDA FoodData Central when online lookups are on. Visual Intelligence feeds the same reader: point Camera Control at food, a label or a menu and pick **Identify from Photo**.
- **Nutrition from a page, a PDF, or a menu** — **Share → Onigiri** from Safari reads nutrition figures on-device from a web page, PDF, link, image, or menu document: a single food fills the form, a whole restaurant guide becomes a searchable list you log from item by item, and AI estimates dishes with no calories printed.
- **AI features (optional — off by default)** — one switch adds nutrition estimates from a description or photo (refinable afterward in your own words, e.g. "no dressing" or "it's tofu, not chicken"), label/menu reading fallbacks, and meal-name suggestions, each reviewed before saving and marked with a small ✨, running on-device with Apple Intelligence or your own Anthropic/OpenAI key or local server ([details](https://ecliptik.github.io/onigiri/privacy/#ai-features-optional)).
...
- **Widgets** — a Today widget (small, medium, large, and extra large on iPad and iOS 27 — with the Active/Resting split and today's log) with your calorie ring and a one-tap Log button, plus a month-stats card, water and streak widgets, and a Control Center water button.
...
**Example.** Your average burn is ~2,500 kcal/day and your goal needs a **500 kcal/day** deficit, leaving a **2,000 kcal budget**. You've eaten 1,850 so far, so the ring reads **150 kcal left** — room for 150 more and you'll still land on your target deficit by bedtime.
...
- Eat nothing more and the deficit only grows — you'd *stop* losing only if you ate all the way up to your full burn (~2,500).

The **Daily goal** card shows the same thing from the deficit side: "350 of 500 kcal deficit" is what counts *so far* — the rest of the day's burn is still coming, which is why that reading and "150 kcal left" point at the same 500 target.
