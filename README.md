...
- **Barcode, label & menu scanning** — one camera, no mode to pick: it reads a nutrition-facts label, turns a photographed menu board into a list, or (with AI) identifies food straight from a photo, with barcode scans and food search filled in from OpenFoodFacts and USDA FoodData Central when online lookups are on.
