...
- **Barcode, label & menu scanning** — one camera, no mode to pick: snap a nutrition-facts label and the values are read on-device, photograph a menu board with printed calories and it becomes a list, or (with AI) photograph the food itself. With online lookups enabled, barcode scans and food search fill in from OpenFoodFacts and USDA FoodData Central.
- **Nutrition from a page, a PDF, or a menu** — eating out, where the numbers live only on the restaurant's website: **Share → Onigiri** from Safari and the figures are read on-device, including the food's name, which a photographed label never carries. Works on a page, a PDF, a link, an image, or a menu document from Files. A single food goes to the form; a table or a whole restaurant guide becomes a searchable list you order from item after item, with a running receipt of what has been logged. Menus with no calories printed on them (most independents) are listed with an estimate beside each dish when AI is on.
- **AI features (optional — off by default)** — turn on one switch and Onigiri can estimate nutrition from a plain-language description ("pork and beans"), fill label values the scanner missed, identify a dish from a photo, read a bakery card or shelf sign that only *names* a food, list what a menu offers, and suggest meal names. Anything estimated can be **refined in your own words** first — "no dressing", "I only ate half", "it's tofu, not chicken" — because with Apple Intelligence the model never sees your photo, only what Apple's on-device vision made of it; a refine that doesn't land keeps the estimate you had. Every estimate reviewed before it's saved, and AI-created foods keep a small ✨ by their name. Runs on-device with Apple Intelligence, or bring your own AI: an Anthropic or OpenAI key, or any local OpenAI-compatible server (Ollama, LM Studio — Gemma, Qwen, and friends). Keys live in the Keychain; details in the [Privacy Policy](https://ecliptik.github.io/onigiri/privacy/#ai-features-optional).
