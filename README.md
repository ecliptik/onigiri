...
- **Nutrition from a screenshot** — eating out, where the numbers live only on the restaurant's website: screenshot the page, *Copy and Delete* so nothing lands in Photos, and paste it into Onigiri. The form fills itself, including the food's name; a page listing several dishes asks which one you meant. The paste row appears only when an image is actually on the clipboard.
