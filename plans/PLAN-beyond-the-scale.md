...
The row today is `Last 30 days` over two lines — "−N lb predicted" /
"−N lb on scale" (`GoalView.swift:362`), and its own comment records that
...
ones (0.67%/week at 300 lb). **For a body near the seeder's it changes
essentially nothing** — the below-resting check is where the value is, and it may be
...
- **F3 — DONE 2026-08-16, below-resting half only.** `plan`/`derivePlan`
  ...
  **The proportional half was NOT built and should not be**, per the
  ...
  at 1.0%/week, so it would change nothing for a body like that while adding a
