...
> logging and burn. [X] here as a budget is different than the budget,
> average day of [Y]." — the user, 2026-08-23, plus: add the kcal-left
...
(Illustrative round figures with the same shape; the user's own stay off
this page.)

    Budget, right now      1,500 kcal
    Eaten today            1,000 kcal
    Burned today           2,000 kcal
    ...
    Deficit needed           500 kcal/day
    Budget, average day  ≈ 2,300 kcal/day

Both figures are the SAME formula, `burn − deficit`, on two different burns:

| row                   | burn                                  | − deficit | = budget |
|-----------------------|---------------------------------------|-----------|----------|
| `Budget, right now`   | today's `DayBudget.dayBurn` = 2,000   | 500       | 1,500    |
| `Budget, average day` | `averageDailyBurnKcal` = 2,800        | 500       | 2,300    |
...
1. **The 800 kcal gap is entirely ACTIVE energy.** Resting is credited whole
...
2. **A set daily budget already exists — it is 2,300.**
...
3. **Whichever budget "kcal left" rides is the one the day is graded on.**
   If Today's ring says 1,300 left (against 2,300) while `DayBadgeRule` /
   the calendar / the streak grade against 1,500, the app invites the reader
...
1. **`Budget, right now` → `Budget, today's burn`.** Keeps the comma form
   ...
   `Burned today` (2,000) sits two rows below and `Deficit needed` (500) is
   one disclosure away, so `2,000 − 500 = 1,500` can be verified without
...
2. **Add `Left today`** — `todayBudget − todayIntakeKcal`, through
   ...
   reads as the subtraction it is (1,500 − 1,000 = 500). This is the user's
...
3. **The footer carries the reconciliation, in live numbers.** The pattern
   the user liked on 2026-08-18 ("N lb over N days needs N kcal/day")
   applied here: after the existing direction clause, one sentence naming the
   forecast and the gap — e.g. "On an average day it finishes near 2,300 kcal;
...
- It restores the trailing-average substitution PLAN-earned-budget deleted,
  with its documented failure: "kcal left" on a day that ended over break-even
  (2026-07-30). A sedentary day passes on a promise it did not earn.
...
("you are 800 kcal below an average day"). Honest, but it is B with a nag
...
decompose the row:
    ...
    Resting budget      1,300 kcal   ← yours at midnight, does not move
    + Earned by moving    200 kcal   ← active so far today
    = Budget              1,500 kcal
...
- **`Left today`** added, directly under the intake row, through
  `remainingHeadline` (`+N kcal over` past the budget, never a negative
