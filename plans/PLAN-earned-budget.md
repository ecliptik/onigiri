...
Today's Details screen showed hundreds of "kcal left" and a "kcal surplus"
at the same time, and July 30 read differently on Calendar vs Details. Not
...
2. **The budget is a full-day allowance from the trailing AVERAGE.** A
   budget built on the average assumed far more burn than Health had
   recorded. So the "left" figure is an allowance against a day you aren't having,
...
Goal's figure and Details' sat hundreds of kcal apart — both right,
indistinguishable. Goal is the average-day projection;
Details is `dayBurn − deficit` on a day that at lunchtime had earned little
active energy and would climb all afternoon.
- Goal's "Daily plan" now names BOTH: **"Average day"** over
  **"Today"**, with a footer saying
...
- The morning acceptance test is unrun: at 8am the ring must read
  ...
  (Partly evidenced already: at lunchtime the budget sat above what the
  measured burn alone allowed, so the estimate IS flooring resting.)
...
- **Badge verdict needs BOTH gates.** `StreakCalendar.isTracked` AND the
  `DayBadgeRule`. Today's goal card ran only the second, so a day logged
  under the untracked threshold read "earned" while the calendar left it
...
- **Calendar day card** shows the ring's own figure through
  `remainingHeadline` ("+N over"), not a bare banked deficit; green is
