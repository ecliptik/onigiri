...
Aug 14, a morning weigh-in came in a fraction under the target (figures
below are illustrative, rebuilt on a synthetic 190 lb target). One
screen said four different things at once:

- **Progress: 10.2 of 10.0 lb**, bar full — done, and then some.
- **"On this trend, you'll hit your target between <date> and <date + 5>"** —
...
| Progress bar / "10.2 of 10.0 lb" | raw last weigh-in | `GoalProgress.resolve(currentWeightLb:)` |
...
`if goalReached` (`GoalView.swift:675`). So at 189.8 against a 190 target
the "Keep going · 5 lb more / 10 lb more" chips do not exist, and the only
route to a 180 target is hand-editing the field — which takes the
`targetChanged` branch in `GoalUpsert.save:97`, re-stamps the start to
today at the RAW last weigh-in, and re-zeros a bar that read 10.2 of 10.0.
...
the basis crosses 190 that takes out the whole "Today — Budget / Burn"
...
- The right-hand end of the blue line does not equal any number on the
  screen — not the 189.8 above it, not the basis under "How budget is
...
target 190 → 180 grows the axis from ~13 lb tall to ~23 lb and visually
...
0 the moment the basis touches 190. `WeightTrend.swift:148` already
...
- `.approaching` — a green (not orange) line: *"Your 7-day average is
  190.8 lb — 0.8 lb from your target. Three weigh-in days in a week
  averaging 190 lb or less finishes it."* Plus the **"Keep going" chips,
...
- **Target change:** NOT capped. The draft wanted to stop a distant
  ...
  "here's the distance" is what that line is for. A 180 lb target on a
  190–199 history draws a ~23 lb axis, and that is the honest picture.
...
- The Aug 14 fixture, in shape: start 200.0, target 190, raw latest 189.8,
  daily lows averaging just above 190 → `.approaching`, NOT
