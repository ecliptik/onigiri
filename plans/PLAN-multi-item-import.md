...
## Round 2 — the prompt that vanished, and an entry you can correct (2026-09-20)

Reported from the phone, with a screen recording: sharing a photo of ONE
menu item "briefly pops up a dialog for the restaurant name but then
disappears", and there was no way to fix what came back — the read had
named the dish after the topping it was configured with ("Plain with
Fish Seasoning"), filed 1 g of trans fat where the page printed 0, and
dropped the page's saturated fat entirely.

### Three causes, measured not guessed

1. **The prompt belonged to the wrong view.** `.alert` hung on
   `MenuPicker`, so it died with the picker the moment `phase` left
   `.picking` — and an `initialPick` (a single shared item) confirms
   itself in the same breath as the ask. The 2026-08-29 fix had moved
   the STATE up to the flow and left the PRESENTER behind.
2. **Raised in the same turn as the phase change it never appeared at
   all.** Moving the `.alert` onto `MenuPickerFlow` was not enough: with
   the ask and the confirm landing in one update, the extension showed
   no dialog whatsoever (frames, 27.0 sim). The flow now awaits the
   initial pick and asks afterwards, so the question is raised over the
   item it is about to name.
3. **The answer never reached a single item anyway.** The source prefix
   was applied in `MenuPicker.choose`, which an `initialPick` never
   passes through. It is `MenuSourceName` in the kit now (idempotent,
   because the prompt is typed into while the confirm is already up, and
   every keystroke re-applies it), applied by the flow for both paths.

### What shipped

- `LogEntryEditor` (app + extension): name, serving, calories and every
  nutrient, in the food form's own groups with its "N filled" counts.
  Pushed from an `Edit Item` row — no Save of its own, because nothing
  is written until the confirm's Save or Log. (It shipped on the item
  row itself, with a caption saying so, and came back the same day:
  "edit isn't obvious, tooltip shows to tap to edit, but could we add an
  explicit Edit button". A caption explaining that a row is tappable is
  the shape of an affordance that isn't one.) Editing a figure
  drops the plausibility finding that judged it: a value the user typed
  was not READ, so neither the ⚠️ beside it nor the "was left out" note
  still applies.
- `LogConfirmSheet.label` became a `@Binding`, its first section gained
  a footer naming the way in, and the receipt's footer stopped saying
  "found" (it is true of a typed figure too now).
- The flow's setup runs once per import (`started`). A NavigationStack
  re-runs a covered root's `.task` when the pushed view pops, which
  re-asked the restaurant, discarded the one already typed, and — with
  an `initialPick` — would have re-run the pick over the correction just
  made.
- `rows.isEmpty` no longer renders the picker: a single shared item
  showed "No items match" over "Search 0 items" for the frames before
  its confirm arrived.

### The landmine this round bought

`Binding($chosen)` — SwiftUI's optional-unwrapping initializer — force
unwraps on every graph update, and this screen clears `chosen` while the
confirm is still on screen (Back does; so does a finished Log). The
process traps: `EXC_BREAKPOINT` in
`BindingOperations.ForceUnwrapping.get`, no banner, the sheet simply
gone one frame after a successful write. Caught on the first Back of the
first simulator run, fixed with a binding whose getter falls back to the
value it was built with.

### Verified

- The kit suite: 756 tests, 0 failures (`MenuSourceNameTests` is 7 of
  them — trimming, idempotence, case, and an unnamed item taking the
  source as its whole name).
- `testMenuPickerLogsSeveralItems` extended and run: backs out of a
  confirm before anything is logged (the trap above), edits the first
  item's name in the editor, and asserts the TYPED name is what the
  picker's note and Health both carry. It does not assert the prompt's
  behaviour — this sample names its cafe, so the prompt never opens and
  the check would pass over a broken build.
- The share extension, on the 27.0 sim, from Photos → Share → Onigiri:
  frames show "Analyzing photo…" → "Preparing the item…" → the confirm →
  **"Where is this from?" standing over it and still there ten seconds
  later**; answering it renamed the item to "Carbs (Cali Fish Grill)";
  the item row pushed "Edit Item" inside the extension.
- In-app, from a menu shared into `MenuImportSheet`: the editor's trans
  fat 0 → 4.5 and calories 620 → 655 reached the confirm and the day's
  log.

### Not verified here

The phone. Synthetic taps land on the extension's window at roughly
2.2× their visual position on this simulator, which is a harness
artifact — it cost an hour of "the form is inert" before a mapped tap
opened the editor. Nothing about the extension's own behaviour should
be inferred from it.
